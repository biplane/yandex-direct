...
* Все константы, относящиеся к кодам ошибок API, будут удалены из класса `ApiException`,
  в версии 5.0.
  Для ошибок, которые обрабатываются, рекомендуется использовать константы на уровне приложения.
