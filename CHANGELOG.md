...
* [**BC break**] Полностью переработана логика генерации кода по данным WSDL.
  Для устранения конфликтов со схемами WSDL.
  Эти изменения частино ломают обратную совместимость, так как до этого некоторые DTO
  некорректно были отображены в PHP.

  * Удалены неиспользуемые DTO для версии V4 Live.
  * Изменено пространство имен для класса `SortOrderEnum`.
    ```diff
    - Biplane\YandexDirect\Api\V5\Contract\SortOrderEnum
    + Biplane\YandexDirect\Api\V5\Reports\SortOrderEnum
    ```
  * Типы, которые являются обертками над массивом, теперь реализуют интерфейс `IteratorAggregate`.
  * Для некоторых свойств уточнены типы.
  * Если свойство является обязательным по схеме, то теперь нельзя установить явно значение `null` через сеттер.
